...
Neural networks have several applications. 

1.	Since the neural network model can analyze the behavior of the person, it can be used to predict the preference of the policy by the public.

2.	Evaluate the policy in energy and public health in the long term by modeling the effect of the policy on the system and predicting the results under it. For example, Dozic, Damir J. and Urosevic, Branka D. Gvozdenac predict the emission of CO2 in the long term with current energy plicies by using the neural network model and evaluate the energy policy based on the prediction. 

3.	The Neural networks can be used in the process of policy-making. We can examine a set of combination of inputs in the model to find the best output under budget constraint.
